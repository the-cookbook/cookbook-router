...
describe('RouterProvider', () => {
  it('runs provider middleware for the current route and writes redirects to history', async () => {
    const router = createMemoryRouter({
      routes: defineRoutes([
        { id: 'private', path: '/private', view: () => <h1>private</h1> },
        { id: 'login', path: '/login', view: () => <h1>login</h1> },
      ] as const),
  ...
  it('does not freeze when replace navigation is requested during render', async () => {
    ...
    const router = createMemoryRouter({
      routes: defineRoutes([
        { id: 'users.show', path: '/users/{slug}', view: MissingUserPage },
        { id: 'not-found', path: '/not-found', view: NotFoundPage },
      ] as const),
  ...
  it('syncs browser-only hashes after the provider has mounted', async () => {
    function HashPage() {
      const hash = useHashParams('article');
      return <p>{`Hash: ${hash ?? 'none'}`}</p>;
    }
  ...
  it('renders route loading fallback when route view suspends', async () => {
  ...
  it('renders route loading fallback inside the parent outlet when route view suspends', async () => {
      ...
      routes: defineRoutes([
        {
          ...
          path: '/',
          view: LazyPage,
          layout: { view: DashboardLayout, loading: DashboardLoading },
        },
      ] as const),
    });
    await router.resolveCurrent();
  ...
  it('keeps same-route layout mounted while its view loads', async () => {
    let resolvePage: ((value: { default: typeof HomePage }) => void) | undefined;
    let mountCount = 0;
    const LazyPage = lazy(
      () =>
      ...
      routes: defineRoutes([
        {
          ...
          path: '/',
          view: LazyPage,
          layout: { view: StableLayout, loading: LayoutLoading },
        },
      ] as const),
    });
    await router.resolveCurrent();
  ...
  it('keeps the same layout view mounted across sibling route navigation', async () => {
    let layoutMounts = 0;
      ...
      routes: defineRoutes([
        {
          ...
          path: '/overview',
          view: HomePage,
          layout: { view: PersistentLayout, loading: LoadingFallback },
        },
        {
          id: 'reports',
          path: '/reports',
          view: AboutPage,
          layout: { view: PersistentLayout, error: ErrorFallback },
        },
      ] as const),
      ...
      routes: defineRoutes([
        {
          ...
          children: [{ id: 'child', index: true, view: LazyPage }],
        },
      ] as const),
    });
    await router.resolveCurrent();
      ...
      routes: defineRoutes([
        {
          id: 'dashboard',
          path: '/',
          layout: { view: StableDashboardLayout, loading: DashboardLoading },
          children: [
            { id: 'dashboard.home', index: true, view: HomePage },
            { id: 'dashboard.reports', path: 'reports', view: LazyReportsPage },
          ],
        },
      ] as const),
    });
      ...
      routes: defineRoutes([
        {
          id: 'dashboard',
          path: '/',
          layout: { view: ShellLayout, loading: DashboardLoading },
          children: [{ id: 'dashboard.home', index: true, view: LazyPage }],
        },
      ] as const),
    });
    await router.resolveCurrent();
      ...
      routes: defineRoutes([
        {
          id: 'dashboard',
          path: '/',
          layout: { view: ShellLayout, loading: LayoutLoading },
          children: [
            {
              id: 'dashboard.home',
              index: true,
  ...
  it('renders global loading fallback when route view suspends without route loading', async () => {
    const LazyPage = lazy(() => new Promise<{ default: typeof HomePage }>(() => undefined));
  ...
  it('renders route error when route view throws', async () => {
      ...
      routes: defineRoutes([
        {
          id: 'root',
          path: '/',
          layout: { view: Layout, error: LayoutErrorFallback },
          children: [{ id: 'broken', index: true, view: BrokenPage }],
        },
      ] as const),
    });
    await router.resolveCurrent();
      ...
      routes: defineRoutes([
        {
          ...
          path: '/',
          view: BrokenPage,
          layout: { view: ErrorLayout, error: LayoutErrorFallback },
        },
      ] as const),
    });
    await router.resolveCurrent();
      ...
      routes: defineRoutes([
        {
          ...
          children: [{ id: 'child', index: true, view: BrokenPage }],
        },
      ] as const),
    });
    await router.resolveCurrent();
